...
public static function find_by_id($id) {
    ...
    if (!$stmt) {
        die("Database query preparation failed. Error: " . $db->connection->error);
    }
    ...
    if (!$result_set) {
        die("Database query execution failed. Error: " . $stmt->error);
    }
...
public static function find_by_name($name) {
    ...
    $sql = "SELECT * FROM " . static::$db_table . " WHERE wedding_type = ?";
    $stmt = $db->connection->prepare($sql);
...
public static function find_by_title($title) {
    ...
    $sql = "SELECT * FROM " . static::$db_table . " WHERE title = ?";
    $stmt = $db->connection->prepare($sql);
 ...
 public static function find_by_user_id($name) {
    // Properly escape the $name variable to avoid SQL injection
    global $db; // Assuming you have a $database object for your DB connection
    $safe_name = $db->escape_string($name);

    $the_result_array = static::find_by_query("SELECT * FROM " . static::$db_table . " WHERE user_name = '{$safe_name}' ORDER BY user_name DESC LIMIT 1");
    return !empty($the_result_array) ? array_shift($the_result_array) : false;
}
...
public static function find_by_query($sql) {
    ...
    if (!$result_set) {
        die("Database query failed. Error: " . mysqli_error($db->connection));
    }
    ...
    public function save_user() {
        return isset($this->user_name) ? $this->update_user() : $this->create();
    }

    public function save_account() {
        return isset($this->user_name) ? $this->update_user_account() : $this->create();
    }
    ...
    public function update_user_account() {
        ...
        $sql .= " WHERE user_name = '" . $db->escape_string($this->user_name) . "'";
        $db->query($sql);
